...
char* rotrHexa(char* instruction) {
	...
	while((instruction[i]==" ")||(instruction[i]=="%t")){ //passage au rotr
		i++;
	}
	while((instruction[i]!=" ")&&(instruction[i]!="%t")){ //passage de l'rotr
		i++;
	}
