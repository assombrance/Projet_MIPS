...
char* subHexa(char* instruction) {
	...
	while((instruction[i]==" ")||(instruction[i]=="%t")){ //passage au sub
		i++;
	}
	while((instruction[i]!=" ")&&(instruction[i]!="%t")){ //passage de l'sub
		i++;
	}
