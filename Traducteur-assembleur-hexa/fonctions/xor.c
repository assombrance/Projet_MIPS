...
char* xorHexa(char* instruction) {
	...
	while((instruction[i]==" ")||(instruction[i]=="%t")){ //passage au xor
		i++;
	}
	while((instruction[i]!=" ")&&(instruction[i]!="%t")){ //passage de l'xor
		i++;
	}
